...
export var props = {
    getStyle: (ele: HTMLElement): Style | undefined => {
        return ele['ele-style-2ca4a51'];
    },
    setStyle: (ele: HTMLElement, style: Style | undefined) => {
        if (!style) return;
        Utils.setStyle(ele, style);
...
export var paragraphProps = {
    ...
    setEleUniId: (ele: HTMLElement, id?: string) => {
        if (id) {
            ele.setAttribute('data-uni-id', id);
        } else {
