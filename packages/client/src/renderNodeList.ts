...
export function renderNodeList(memloss: MemLoss, list: Array<Node>, level: number, elemtCreated: (item: HTMLElement) => void): Array<HTMLElement> {
    ...
    for (let node of list) {
        ...
        const subopen = Utils.anonyFunction((self: HTMLElement) => {
            if (item.get('opend')) {
                ...
                if (node.children) {
                    renderNodeList(memloss, node.children, level + 1, item => $(children).appendChild(item));
                }
            }
        })
        item.innerHTML = `
